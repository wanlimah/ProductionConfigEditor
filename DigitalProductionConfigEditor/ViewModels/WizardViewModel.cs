    ...
    public class AttributeViewModel : INotifyPropertyChanged
    {
        ...
        { 
            ...
            set 
            {
                ...
                // Validate numeric fields - only allow digits during user input
                var numericFields = new[] { "count", "sampling", "threshold", "x", "y" };
                if (!string.IsNullOrEmpty(_name) && numericFields.Contains(_name.ToLower()))
                {
                    // Only keep digits for numeric fields
                    var numericValue = new string(value.Where(char.IsDigit).ToArray());
                    _value = numericValue;
                }
                else
                {
                    // Accept any text for non-numeric fields
                    _value = value;
                }
                OnPropertyChanged(nameof(Value));
            }
        ...
        public List<string>? Options { get; set; } // If this attribute has predefined options
    ...
    public class WizardViewModel : INotifyPropertyChanged
    {
        ...
        private void LoadAttributesFromSelectedNode()
        {
            ...
            foreach (XmlAttribute attr in SelectedPackageNode.Attributes)
            {
                ...
                attrViewModel.Name = attr.Name;
                attrViewModel.Value = attr.Value;

                // Check if there are predefined options for this attribute
        ...
        public void SaveAttributesToNode()
        {
            ...
            {
                if (SelectedPackageNode.Attributes[attr.Name] != null)
                {
                    SelectedPackageNode.Attributes[attr.Name]!.Value = attr.Value;
                }
            }
        }
